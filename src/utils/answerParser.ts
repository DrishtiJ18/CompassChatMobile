...
type Approaches = typeof Approaches;
type HtmlParsedAnswer = {
    content: string;
    work_citations: string[];
    web_citations: string[];
    work_sourceFiles: Record<string, string>| string[];
...
// console.log("answer",work_citation_lookup)
// console.log("answer",web_citation_lookup)
                ...
                if (!pattern.test(part)) {
                   ...
                   // Odd parts are citations as the "FileX" moniker
                        ...
                        if (!citation) {
                            ...
                            citationShortName = citation_lookup[part]?.citation
                                .split('/')
                                .slice(4)
                                .join('/');
                          ...
                          //   console.log("url ",encodeURI(citation.source_path))
                            return `[${citationIndex}](${encodeURIComponent(citation.source_path)} "FileName: ${citationShortName}, PageNumber: ${pageNumbers[citationShortName]}, type:work")`
                            // return `[${citationIndex}${citation.citation}]`
                    ...
                    if (approach == Approaches.ChatWebRetrieveRead) {
                        ...
                        // Odd parts are citations as the "FileX" moniker
                        const citation = citation_lookup?.[part];
                        citationShortName = citation?.citation
                        if (!citation) {
                            // if the citation reference provided by the OpenAI response does not match a key in the citation_lookup object
                            // then return an empty string to avoid a crash or blank citation
                            // console.log('citation not found for: ' + part);
                          ...
                          return `[${citationIndex}](${(citation.citation)} "FileName: ${citationShortName}, PageNumber: ${citation.page_number}, type:web")`
                        }
                    }
                    return '';
            ...
            work_fragments = work_parts.map((part, index) => {
                ...
                if (index % 2 === 0) {
                    ...
                    if (!work_citation) {
                        ...
                        // Check if the citation is already in the citations array
                        if (
                            work_citations.includes(
                                work_citation.citation.split("/").slice(4).join("/")
                            )
                        ) {
                            ...
                            work_sourceFiles[
                                work_citation.citation.split("/").slice(4).join("/")
                            ] = work_citation.source_path;
                            pageNumbers[work_citation.citation] = Number(
                                work_citation.page_number
                            );
                        }
                    }
                }
        ...
        console.log('answer parser error==', error);
    }
}